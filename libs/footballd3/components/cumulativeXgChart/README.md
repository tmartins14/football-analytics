...
Produced by: `libs/statsbomb/extract_cumulative_xg.py`
Consumes: shot records from `extract_shots(match_id)` (in-process — no intermediate file)
Produces: `libs/footballd3/sample_data/cumulative_xg_{match_id}.json`
...
`libs/footballd3/sample_data/cumulative_xg_{match_id}.json`
